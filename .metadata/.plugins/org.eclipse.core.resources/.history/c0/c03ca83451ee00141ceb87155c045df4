...
void xMIPv6::sendPeriodicBU(cMessage *msg)
{
	...
	//PROXY UNLOADING LOGGING:
	//LOG THE BUs
	MyFileWriter* writer = new MyFileWriter();
	writer->printOutBindingUpdates(rt6->getHomeNetHoA_adr().str().c_str(),buAdrCoA.str().c_str(),buDest.str().c_str(), simTime().str().c_str() );
	//'######################################################
	   ...
	   //***********************PROXY UNLOADING********************
	    ...
	    //LOG THE INFOS:
	    writer->printOutFBUs(buAdrCoA.str().c_str(), simTime().str().c_str() );
	   ...
	   // if(strcmp(this->lastSendToFlowBindingAddress.c_str(),CoA.str().c_str())){//if it is not the same - send new binding Update message - strcmp returns 0 when the same
	    //if(!strcmp(ha.str().c_str(),dest.str().c_str())){
	        ...
	        newFlowBindingUpdateToSend->setNewCoAdress(buAdrCoA.str().c_str());
	        ...
	        this->lastSendToFlowBindingAddress= buAdrCoA.str();
