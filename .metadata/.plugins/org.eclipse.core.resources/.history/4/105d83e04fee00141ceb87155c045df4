...
void AbstractRadioExtended::initialize(int stage) {
    ...
    if (stage == 0) {
        ...
        if (transmitterPower > (double) (cc->par("pMax")))
            error(
                    "transmitterPower cannot be bigger than pMax in ChannelControl!");
        ...
        EV << "Initialized channel with noise: " << noiseLevel
        ...
        if (ccExt) {
            ((ChannelControlExtended::HostRefExtended) myHostRef)->registerRadio(
                    this);
            ccExt->updateHostChannel(myHostRef, rs.getChannelNumber(), this,
                    carrierFrequency);
        } else
    ...
    if (ccExt && airframeext) {
        ...
        if (fqFrame > 0.0 && carrierFrequency > 0.0) {
            if (chnum == getChannelNumber()
                    && (fabs((fqFrame - carrierFrequency) / carrierFrequency)
                            <= perc))
                return true;
            else
                return false;
...
void AbstractRadioExtended::handleMessage(cMessage *msg) {
    ...
    if (msg->getArrivalGateId() == uppergateIn
            && !msg->isPacket() /*FIXME XXX ENSURE REALLY PLAIN cMessage ARE SENT AS COMMANDS!!! && msg->getBitLength()==0*/) {
        cPolymorphic *ctrl = msg->removeControlInfo();
        if (msg->getKind() == 0)
            error(
                    "Message '%s' with length==0 is supposed to be a command, but msg kind is also zero",
                    msg->getName());
    ...
    if (msg->getArrivalGateId() == uppergateIn) {
        ...
        handleSelfMsg(msg);
    } else if (processAirFrame (check_and_cast<AirFrame*>(msg))) {if (this->isEnabled())
    {
        // must be an AirFrame
...
void AbstractRadioExtended::bufferMsg(AirFrame *airframe) //FIXME: add explicit simtime_t atTime arg?
    ...
    scheduleAt(airframe->getArrivalTime() + airframe->getDuration(),
            endRxTimer);
}

AirFrameExtended *AbstractRadioExtended::encapsulatePacket(cPacket *frame) {
    PhyControlInfo *ctrl =
            dynamic_cast<PhyControlInfo *>(frame->removeControlInfo());
    ...
    if (ctrl && ctrl->getAdativeSensitivity())
        updateSensitivity(ctrl->getBitrate());
    ...
    EV << "Frame (" << frame->getClassName() << ")" << frame->getName()
              << " will be transmitted at " << (airframe->getBitrate() / 1e6)
              << "Mbps\n";
    return airframe;
}
...
void AbstractRadioExtended::handleUpperMsg(AirFrameExtended *airframe) {
    if (rs.getState() == RadioState::TRANSMIT)
        error(
                "Trying to send a message while already transmitting -- MAC should "
                        "take care this does not happen");

    // if a packet was being received, it is corrupted now as should be treated as noise
    if (snrInfo.ptr != NULL) {
        EV
                  << "Sending a message while receiving another. The received one is now corrupted.\n";
    ...
    if (msgkind == PHY_C_CONFIGURERADIO) {
        ...
        if (newChannel != -1) {
            ...
            if (rs.getChannelNumber() == newChannel)
                ...
                EV
                          << "We're transmitting right now, remembering to change after it's completed\n";
                this->newChannel = newChannel;
            } else
                changeChannel(newChannel); // change channel right now
        }
        if (newBitrate != -1) {
            EV << "Command received: change bitrate to " << (newBitrate / 1e6)
                      << "Mbps\n";
...
void AbstractRadioExtended::handleSelfMsg(cMessage *msg) {
    ...
    if (msg->getKind() == MK_RECEPTION_COMPLETE) {
        ...
        if (noiseLevel < sensitivity) {
            ...
            EV
                      << "transmission over but noise level too high, switch to recv mode (state:RECV)\n";
...
void AbstractRadioExtended::handleLowerMsgStart(AirFrame* airframe) {
    ...
    double rcvdPower = receptionModel->calculateReceivedPower(
            airframe->getPSend(), frequency, distance);
    if (obstacles && distance > MIN_DISTANCE)
        rcvdPower = obstacles->calculateReceivedPower(rcvdPower,
                carrierFrequency, framePos, 0, myPos, 0);
    ...
    if (airframe->getArrivalTime()
            == simTime() && rcvdPower >= sensitivity && rs.getState() != RadioState::TRANSMIT && snrInfo.ptr == NULL) {
...
void AbstractRadioExtended::handleLowerMsgEnd(AirFrameExtended * airframe) {
    ...
    if (snrInfo.ptr == airframe) {
        ...
        if (!radioModel->isReceivedCorrectly(airframe, list)) {
            ...
            airframe->getEncapsulatedPacket()->setKind(
                    list.size() > 1 ? COLLISION : BITERROR);
#else
        ...
        if ((numReceivedCorrectly + numGivenUp) % 50 == 0) {
            lossRate = (double) numGivenUp
                    / ((double) numReceivedCorrectly + (double) numGivenUp);
    ...
    if (noiseLevel
            < sensitivity&& rs.getState() == RadioState::RECV && snrInfo.ptr == NULL) {
...
void AbstractRadioExtended::changeChannel(int channel) {
    ...
    if (channel < 0 || channel >= cc->getNumChannels())
        error(
                "changeChannel(): channel number %d is out of range (hint: numChannels is a parameter of ChannelControl)",
                channel);
    if (rs.getState() == RadioState::TRANSMIT)
    ...
    if (rs.getState() == RadioState::RECV) {
        ...
        for (RecvBuff::iterator it = recvBuff.begin(); it != recvBuff.end();
                ++it) {
    ...
    if (ccExt) {
        ChannelControlExtended::TransmissionList tlAux =
                ccExt->getOngoingTransmissions(channel);
        for (ChannelControlExtended::TransmissionList::const_iterator it =
                tlAux.begin(); it != tlAux.end(); ++it) {
            AirFrameExtended *airframe = check_and_cast<AirFrameExtended *>(
                    *it);
            ...
            if (airframe->getTimestamp() + propagationDelay >= simTime()) {
                ...
                sendDirect(airframe->dup(),
                        airframe->getTimestamp() + propagationDelay - simTime(),
                        airframe->getDuration(), myHost, radioGate->getId());
            }
            // if we hear some part of the message
            else if (airframe->getTimestamp() + airframe->getDuration()
                    + propagationDelay > simTime()) {
                EV << "missed beginning of frame, processing it as noise\n";

                AirFrameExtended *frameDup = airframe->dup();
                frameDup->setArrivalTime(
                        airframe->getTimestamp() + propagationDelay);
        ...
        ChannelControl::TransmissionList tlAux = cc->getOngoingTransmissions(
                channel);
        for (ChannelControl::TransmissionList::const_iterator it =
                tlAux.begin(); it != tlAux.end(); ++it) {
            ...
            if (channel == airframe->getChannelNumber()) {
                ...
                if (airframe->getTimestamp() + propagationDelay >= simTime()) {
                    ...
                    for (int i = 0; i < radioGate->size(); i++)
                        sendDirect(airframe->dup(),
                                airframe->getTimestamp() + propagationDelay
                                        - simTime(), airframe->getDuration(),
                                myHost, radioGate->getId() + i);
                }
                // if we hear some part of the message
                else if (airframe->getTimestamp() + airframe->getDuration()
                        + propagationDelay > simTime()) {
...
void AbstractRadioExtended::disconnectReceiver() {
    if (ccExt) {
        ((ChannelControlExtended::HostRefExtended) myHostRef)->unregisterRadio(
                this);
    }
}
...
void AbstractRadioExtended::registerBattery() {
    ...
    if (bat) {
        ...
        if (mUsageRadioIdle < 0 || mUsageRadioRecv < 0 || mUsageRadioSleep < 0
                || mUsageRadioSend < 0)
            return;
        bat->registerWirelessDevice(rs.getRadioId(), mUsageRadioIdle,
                mUsageRadioRecv, mUsageRadioSend, mUsageRadioSleep);
    }
}
...
double AbstractRadioExtended::calcDistFreeSpace() {
    ...
    interfDistance = pow(
            waveLength * waveLength * transmitterPower
